angular.module("angular-msg").provider("msg", function () {
  "use strict";
  ...
  /**
   * set wheter the notficiation is displayed inline our in msg like fasion
   *
   ...
   *
   */
  this.serverMessagesInterceptor = ['$q', 'msg', function ($q, msg) {
    function checkResponse (response) {
      if (response !== undefined && response.data[_messagesKey] && response.data[_messagesKey].length > 0) {
        msg.addServerMessages(response.data[_messagesKey]);
      }
    }
  ...
  this.$get = ["$rootScope", "$interpolate", "$sce", "$filter", "$timeout", "msgMessages", function ($rootScope, $interpolate, $sce, $filter, $timeout, msgMessages) {
    var translate;

    msgMessages.onlyUnique = _onlyUniqueMessages;
    msgMessages.reverseOrder = _reverseOrder;
    ...
    function broadcastMessage (message) {
      ...
      var addedMessage = msgMessages.addMessage(message);
      $rootScope.$broadcast("msgMessage", message);
      $timeout(function () {
      }, 0);
      return addedMessage;
    }
